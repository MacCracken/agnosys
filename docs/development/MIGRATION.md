...
All modules from `userland/agnos-sys/src/` have been copied to `agnosys/src/` (agent/llm later moved to agnosai/hoosh). The `lib.rs` wires them with feature gates. The `Cargo.toml` has all dependencies declared.
...
2. **`agnos_common` dependency removed** — Agent types moved to agnosai crate; LLM types moved to hoosh crate. No external AGNOS crate dependencies.
3. **Cross-module references** — Feature-gate-aware imports working across all modules.
