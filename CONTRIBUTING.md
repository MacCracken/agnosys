...
- [Cyrius](https://github.com/MacCracken/cyrius) toolchain pinned in `cyrius.cyml` (currently **5.10.34** at 1.2.4; install via `curl -sSfL https://raw.githubusercontent.com/MacCracken/cyrius/main/scripts/install.sh | sh`, then `cyriusly use $(grep -oP '(?<=^cyrius = ")[^"]+' cyrius.cyml)`)
- Linux x86_64 (primary host) or aarch64 (cross-build via `cc5_aarch64`); macOS / Windows is a cyrius-side roadmap item
